...
export class MutableCollection {
  ...
  updateKnownTags(tags, getTagRecord) { // Specifies or updates the set of tags for which the collection must have at least POJO records.
    ...
    if (this.knownTags.toString() === tags.toString()) return;
    this.knownTags = tags; // Resets any referencing rules.
  }
  get liveTags() { // The ordered list of tags last provided to updateLiveTags.
    return [];
  }
  updateLiveTags(tags, getTagModel) {    
    const liveTags = this.liveTags,
	  liveLength = liveTags.length,
	  knownTagsSet = this._knownTagSet ||= new Set(),
  ...
  _setRecordValue(tag, promiseOrValue) {
    ...
    if (thenable) promiseOrValue.then(model => this[tag] = model);
  }
}
