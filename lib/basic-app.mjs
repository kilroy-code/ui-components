...
export class BasicApp extends MDElement {
  ...
  get userEffect() {
    return this.avatarElement.src = this.getUserPictureURL();
  }
  ...
  getUserPictureURL(key = this.user) {
    return this.getPictureURL(this.userCollection[key]?.picture);
  }
  get htmlElement() {
  ...
  afterInitialize() {
    ...
    this.shadow$('#user').collection = new MutableCollection({records: userMenuItems});
    this.shadow$('#navigation').collection = new MutableCollection({records: this.screens.filter(s => !userMenuItems.includes(s))});
    const tabs = this.shadow$('menu-tabs');
    tabs.collection = new MutableCollection({records: this.screens});
    tabs.visibleScreenKeys = this.shadow$('slot:not([name]').assignedElements().filter(e => !(e instanceof ListDivider)).map(e => e.title);
