...
export class BasicApp extends MDElement {
  ...
  afterInitialize() {
    ...
    this.shadow$('#navigation').collection = new MutableCollection({
      records: this.screens.filter(s => !userMenuItems.includes(s) && !additionalHeaderItems.includes(s) && !additionalScreenItems.includes(s))
    });
    ...
    if (!this.firstUseScreen?.seen) setTimeout(() => this.displayScreen(this.firstUseScreen.title));
