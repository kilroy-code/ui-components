...
export class AuthorizeUser extends MDElement {
  ...
  static async adopt(tag) {
    const local = App.userCollection.liveTags;
    App.userCollection.addRecordRule(tag, await App.userCollection.getLiveRecord(tag));
    ...
    App.resetUrl({screen: App.defaultScreenTitle, user: tag});
    return tag;
  }
  async onaction() {
  ...
  afterInitialize() {
    ...
    submit.addEventListener('click', async event => {
      ...
      event.target.toggleAttribute('disabled', false);
    });
  }
  get template() {
    return `
